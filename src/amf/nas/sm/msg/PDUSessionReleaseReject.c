...
int decode_pdu_session_release_reject( pdu_session_release_reject_msg *pdu_session_release_reject, uint8_t* buffer, uint32_t len)
{
    ...
    if((decoded_result = decode_extended_protocol_configuration_options (&pdu_session_release_reject->extendedprotocolconfigurationoptions, 0, buffer+decoded,len-decoded))<0)
        return decoded_result;
    else
        decoded+=decoded_result;
...
int encode_pdu_session_release_reject( pdu_session_release_reject_msg *pdu_session_release_reject, uint8_t* buffer, uint32_t len)
{
    ...
    if((encoded_result = encode_extended_protocol_configuration_options (pdu_session_release_reject->extendedprotocolconfigurationoptions, 0, buffer+encoded,len-encoded))<0)
        return encoded_result;
    else
        encoded+=encoded_result;
