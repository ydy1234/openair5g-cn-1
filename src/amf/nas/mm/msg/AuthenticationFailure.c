  ...
  uint32_t len)
{
   ...
   */
  if ((decoded_result = decode_mm_cause (&authentication_failure->mmcause, 0, buffer + decoded, len - decoded)) < 0)
  ...
  uint32_t len)
{
  ...
  if ((encode_result = encode_mm_cause (&authentication_failure->mmcause, 0, buffer + encoded, len - encoded)) < 0)   //Return in case of error
