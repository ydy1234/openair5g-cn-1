...
int encode_plmn_list ( PLMNList plmnlist, uint8_t iei, uint8_t * buffer, uint32_t len  ) 
{
    ...
    if ((encode_result = encode_bstring (plmnlist, buffer + encoded, len - encoded)) < 0)//加密,实体,首地址,长度
...
int decode_plmn_list ( PLMNList * plmnlist, uint8_t iei, uint8_t * buffer, uint32_t len  ) 
{
    ...
    if((decode_result = decode_bstring (plmnlist, ielen, buffer + decoded, len - decoded)) < 0)
